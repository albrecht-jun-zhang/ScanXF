...
{
    class CameraFragment : Fragment, TextureView.ISurfaceTextureListener, Detector.IProcessor
    {
        CameraDevice device;
        ...
        public void UpdateRepeatingRequest()
        {
            ...
            try
            {
                ...
                {
                    barcodeDetector = new BarcodeDetector.Builder(Activity).SetBarcodeFormats(BarcodeFormat.Pdf417).Build();
                    cameraSource = new CameraSource.Builder(Activity, barcodeDetector).SetRequestedPreviewSize(640, 480).SetAutoFocusEnabled(true).Build();
        ...
        public void ReceiveDetections(Detector.Detections detections)
        {
            ...
            if (barcodes.Size() != 0)
            {
                ...
                for (int i = 0; i < barcodes.Size(); i++)
                {
                    ...
                    var barcode = (Barcode)barcodes.Get(key);
                    stringBuilder.Append(barcode.DisplayValue);
                    Log.Info("USPSSurfaceViewRenderer", $"jun: {barcode.DisplayValue}");
                }
                ...
                Device.BeginInvokeOnMainThread(async () => {
                    if (firstTimeDetected)
                    {
                        ...
                        await App.Current.MainPage.Navigation.PushAsync(new ScanResultPage() { ScanResult = stringBuilder.ToString() });
                    }
                });
